...
2. Every **newly constructed** typed error has a **Subtype** — a stable
   ...
   Undeclared subtypes fail CI. Exception during stage 1: errors promoted
   from legacy `*core.ConfigError` via `internal/errcompat` may omit
   Subtype; the gap closes when `core.ConfigError` itself is retired.
